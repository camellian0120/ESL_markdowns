...
予測を正確にするためには、**損失関数(loss function)** $L(X, f(X))$を定める必要があり、中でも**2乗誤差損失(squared error loss)** $L(X, f(X)) = (Y - f(X))^2$は有用である。\
...
[2] 最近傍法とk近傍法: Pythonでの実装や違いについて - PyDocument (参照:2025/04/23)\
https://pydocument.hatenablog.com/entry/2023/10/14/004008#%E6%9C%80%E8%BF%91%E5%82%8D%E6%B3%95Nearest-Neighbor

[3] Pythonで有界な（閉じた）ボロノイ図を計算・描画する #scipy - Qiita (参照:2025/04/23)\
